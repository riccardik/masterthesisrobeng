...
def get_model(n_inputs, n_outputs):
  ...
  model.add(layers.LSTM(128, input_shape=(None, 6), return_sequences=True))
  ...
  return model
def get_model(n_inputs, n_outputs):
  ## two lstm networks increasing learning rate and size
  model = tf.keras.Sequential()
  model.add(layers.LSTM(256, input_shape=(None, 6), return_sequences=True))
  ...
  model.add(layers.Dropout(0.2))
  model.add(Dense(n_outputs, activation='relu'))
  # model.compile(loss='mean_absolute_error', optimizer=Adam(lr=0.05, decay=1e-6),  metrics=['accuracy'] )
  model.summary()
  return model
...
units = 128
output_size = 162  # labels are from 0 to 9
...
fname_model = "./"+"ltsmcomple0035withscalingtraining.h5"
